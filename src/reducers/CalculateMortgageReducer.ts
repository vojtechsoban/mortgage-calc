...
import { calculate } from 'src/service/MortgageCalc'
import { MortgageState } from '../types/state'

// @ts-ignore
export default (state: MortgageState = null, action) => {
  ...
  switch (action.type) {
    ...
    case actionTypes.UPDATE_MORTGAGE_START: {
      ...
      if (result.mortgage && result.mortgage.installments.length > 0) {
    ...
    case actionTypes.SAVE_EXTRA_PAYMENT: {
      ...
      const paymentIndex = state.editingExtraPayment && state.editingExtraPayment.paymentIndex || 0
      ...
      result.editingExtraPayment = undefined
