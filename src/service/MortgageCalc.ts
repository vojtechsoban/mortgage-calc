...
import { InstallmentSum, Mortgage, MortgageParameters } from 'src/models/Mortgage'

export const calculateMonthlyPayment = (principal: number, rate: number, payment?: number) => {
...
export const getMortgageParameters = (mortgage, paymentIndex) => {
  ...
  if (mortgage.parameters.length === 1) {
    ...
    let paymentSum = 0
    for (const parameters of mortgage.parameters) {
...
export const pushMortgageParameters = (mortgage: Mortgage, paymentIndex: number, parameters: MortgageParameters) => {
...
export const getExtraPayment = (mortgage, paymentIndex) => {
  ...
  for (const extraPayment of mortgage.extraPayments) {
...
export const calculate = (mortgageIn) => {
  ...
  while (balance > 0 && count++ <= 1000) {
    ...
    const { installmentPart, principalPart } = calculateMonthlyPayment(balance, rate, payment)
    ...
    if (extraPayment) {
      ...
      const absoluteExtraPayment = extraPayment.amount > 1 ? extraPayment.amount : extraPayment.amount * balance
